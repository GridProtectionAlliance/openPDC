...
- [Where can I find the installer for the PMU Connection Tester?](FAQ.md#question-where-can-i-find-the-installer-for-the-pmu-connection-tester)
- [Does the openPDC provide build scripts?](FAQ.md#question-does-the-openpdc-provide-build-scripts)
- [Can I run the openPDC on a 64-bit OS?](FAQ.md#question-can-i-run-the-openpdc-on-a-64-bit-os)
- [Why does the openPDC include x86 platform targets?](FAQ.md#question-why-does-the-openpdc-include-x86-platform-targets)
- [Can I run the openPDC on low-performance machines?](FAQ.md#question-can-i-run-the-openpdc-on-low-performance-machines)
- [Synchrophasor Questions](FAQ.md#synchrophasor-questions)
    - [openPDC Questions](FAQ.md#openpdc-questions)
        - [What happens to a device when you set a device's "Connection is to Concentrator" property to true?](FAQ.md#question-what-happens-to-a-device-when-you-set-a-devices-connection-is-to-concentrator-property-to-true)
        - [What is the difference between the initial data set and the sample data set?](FAQ.md#question-what-is-the-difference-between-the-initial-data-set-and-the-sample-data-set)
        - [I set up the SQL Server Management Studio and executed the SQL files, but how do I see the changes I made?](FAQ.md#question-i-set-up-the-sql-server-management-studio-and-executed-the-sql-files-but-how-do-i-see-the-changes-i-made)
        - [Does the Device ACRONYM need to be unique? The modeling instructions don't say so, but they do say that it is how the device is referenced by the program.](FAQ.md#question-does-the-device-acronym-need-to-be-unique-the-modeling-instructions-dont-say-so-but-they-do-say-that-it-is-how-the-device-is-referenced-by-the-program)
        - [What are the status flags measurements that are defined for each device?](FAQ.md#question-what-are-the-status-flags-measurements-that-are-defined-for-each-device)
        - [Does the SignalReference field in the Measurement table need to be unique?](FAQ.md#question-does-the-signalreference-field-in-the-measurement-table-need-to-be-unique)
        - [I receive an error like the following: "Invalid pointer address"](FAQ.md#question-i-receive-an-error-like-the-following-invalid-pointer-address)
        - [Would data from a single PMU Device typically be used at more than one Substation? Or is a particular PMU strongly associated with a single Substation?](FAQ.md#question-would-data-from-a-single-pmu-device-typically-be-used-at-more-than-one-substation-or-is-a-particular-pmu-strongly-associated-with-a-single-substation)
        - [When the openPDC is collecting data from other PDCs, is the the parent (concentrator) device or the child device associated with a measurement?](FAQ.md#question-when-the-openpdc-is-collecting-data-from-other-pdcs-is-the-the-parent-concentrator-device-or-the-child-device-associated-with-a-measurement)
        - [Is it possible to configure the minimum and maximum time delays that the openPDC waits for the data of the PMUs in the system arrive (also known as the "resynchronization buffer")?](FAQ.md#question-is-it-possible-to-configure-the-minimum-and-maximum-time-delays-that-the-openpdc-waits-for-the-data-of-the-pmus-in-the-system-arrive-also-known-as-the-resynchronization-buffer)
        - [I cannot get a setting I entered into the configuration file to save. When the application runs it keeps coming up blank and getting removed from the file. What's wrong?](FAQ.md#question-i-cannot-get-a-setting-i-entered-into-the-configuration-file-to-save-when-the-application-runs-it-keeps-coming-up-blank-and-getting-removed-from-the-file-whats-wrong)
        - [Does the openPDC support redundant configurations?](FAQ.md#question-does-the-openPDC-support-redundant-configurations)
        - [What is the difference between Active, Passive or Hybrid connections as it appears in device status under Connection Type?](FAQ.md#question-what-is-the-difference-between-active-passive-or-hybrid-connections-as-it-appears-in-device-status-under-connection-type)
        - [I receive SQL Server connection or authentcation errors.](FAQ.md#question-i-receive-sql-server-connection-or-authentcation-errors)
        - [When setting up a SQLite database, I receive a message like the following, "Could not load file or assembly 'System.Data.SQLite' or one of its dependencies."](FAQ.md#question-when-setting-up-a-sqlite-database-i-receive-a-message-like-the-following-could-not-load-file-or-assembly-system-data-sqlite-or-one-of-its-dependencies)
        - [I need to reset my database. What should I do?](FAQ.md#question-i-need-to-reset-my-database-what-should-i-do)
        - [I receive an error like the following when using an Access Database: (openPDC Manager::AccessDenied)](FAQ.md#question-i-receive-an-error-like-the-following-when-using-an-access-database-openpdc-manager-access-denied)

    - [openPDCManager Questions](FAQ.md#openpdcmanager-questions)
        - [When I run the openPDC Manager, I receive an error that states my operating system does not support SHA-2 algorithms. How can I fix this?](FAQ.md#question-when-i-run-the-openpdc-manager-i-receive-an-error-that-states-my-operating-system-does-not-support-sha-2-algorithms-how-can-i-fix-this)
        - [I am having trouble installing Silverlight Tools on my system. What does "The hash value is not correct" mean?](FAQ.md#question-i-am-having-trouble-installing-silverlight-tools-on-my-system-what-does-the-hash-value-is-not-correct-mean)
        - [I receive an error saying "The installer was interrupted before openPDC Manager could be installed. You need to restart the installer to try again." But when I restart, it simply tells me the same thing. What should I do?](FAQ.md#question-i-receive-an-error-saying-the-installer-was-interrupted-before-openpdc-manager-could-be-installed-you-need-to-restart-the-installer-to-try-again-but-when-i-restart-it-simply-tells-me-the-same-thing-what-should-i-do)
        - [When I try to install openPDC, I recieve an error "Cannot delete the existing openPDC."](FAQ.md#question-when-i-try-to-install-openpdc-i-recieve-an-error-cannot-delete-the-existing-openpdc)
        - [I just installed the openPDCManager. Why isn't it working properly?](FAQ.md#question-i-just-installed-the-openpdcmanager-why-isnt-it-working-properly)
        - [When I go to the webpage, I just get a white screen with an "error on page" icon on the statusbar.](FAQ.md#question-when-i-go-to-the-webpage-i-just-get-a-white-screen-with-an-error-on-page-icon-on-the-statusbar)
        - [The openPDCManager doesn't appear to be connecting to my database.](FAQ.md#question-the-openpdcmanager-doesnt-appear-to-be-connecting-to-my-database)
        - [Why am I receiving a "CrossDomainError"?](FAQ.md#question-why-am-i-receiving-a-crossdomainerror)
        - [What is the purpose of the local checkbox on the Manage Historians? Are there cases where they would not on the local network? What does this change when active?](FAQ.md#question-what-is-the-purpose-of-the-local-checkbox-on-the-manage-historians-are-there-cases-where-they-would-not-on-the-local-network-what-does-this-change-when-active)
        - [The specified service does not exist as an installed service](FAQ.md#question-the-specified-service-does-not-exist-as-an-installed-service)

- [Historian Questions](FAQ.md#historian-questions)
    - [I receive network permissions or HTTP related errors](FAQ.md#question-i-receive-network-permissions-or-http-related-errors)
    - [I noticed that my history file seems to stay the same size all of the time. Does it only store a certain amount of history?](FAQ.md#question-i-noticed-that-my-history-file-seems-to-stay-the-same-size-all-of-the-time-does-it-only-store-a-certain-amount-of-history)
    - [How does the openPDC handle situations where a historian is not responding?](FAQ.md#question-how-does-the-openpdc-handle-situations-where-a-historian-is-not-responding)
    - [How does the openPDC handle the locally cached measurements?](FAQ.md#question-how-does-the-openpdc-handle-the-locally-cached-measurements)

- [Hadoop Questions](FAQ.md#hadoop-questions)
    - [What is Hadoop?](FAQ.md#question-what-is-hadoop)
    - [How does Hadoop relate to the openPDC?](FAQ.md#question-how-does-hadoop-relate-to-the-openpdc)
    - [How are openPDC and Hadoop used in production?](FAQ.md#question-how-are-openpdc-and-hadoop-used-in-production)

### *Question -*  Where can I find the installer for the PMU Connection Tester?
...
#### *Question -*  Does the openPDC provide build scripts
...
#### *Question -*  Can I run the openPDC on a 64-bit OS?
...
#### *Question -*  Why does the openPDC include x86 platform targets?
...
#### *Question -*  Can I run the openPDC on low-performance machines?
...
#### *Question -* What happens to a device when you set a device's *Connection is to Concentrator* property to true?
...
#### *Question -*  What is the difference between the initial data set and the sample data set?
...
#### *Question -*  I set up the SQL Server Management Studio and executed the SQL files, but how do I see the changes I made?
...
#### *Question -* Does the Device ACRONYM need to be unique? The modeling instructions don't say so, but they do say that it is how the device is referenced by the program.
...
#### *Question -*  What are the status flags measurements that are defined for each device?
...
#### *Question -*  Does the SignalReference field in the Measurement table need to be unique?
...
#### *Question -*  I receive an error like the following: "Invalid pointer address"
...
#### *Question -*  Would data from a single PMU Device typically be used at more than one Substation? Or is a particular PMU strongly associated with a single Substation?
...
#### *Question -*  When the openPDC is collecting data from other PDCs, is the the parent (concentrator) device or the child device associated with a measurement?
...
#### *Question -*  Is it possible to configure the minimum and maximum time delays that the openPDC waits for the data of the PMUs in the system arrive (also known as the "resynchronization buffer")?
...
#### *Question -*  I cannot get a setting I entered into the configuration file to save. When the application runs it keeps coming up blank and getting removed from the file. What's wrong?
...
#### *Question -*  Does the openPDC support redundant configurations?
...
#### *Question -*  What is the difference between Active, Passive or Hybrid connections, as it appears in device status under Connection Type?
...
#### *Question -*  I receive SQL Server connection or authentcation errors.
...
#### *Question -*  When setting up a SQLite database, I receive a message like the following, "Could not load file or assembly 'System.Data.SQLite' or one of its dependencies".
...
#### *Question -*  I need to reset my database. What should I do?
...
#### *Question -* I receive an error like the following when using an Access Database: "Access Denied"
...
#### *Question -*  When I run the openPDC Manager, I receive an error that states my operating system does not support SHA-2 algorithms. How can I fix this?
...
#### *Question -*  I am having trouble installing Silverlight Tools on my system. What does "The hash value is not correct" mean?
...
#### *Question -*  I receive an error saying "The installer was interrupted before openPDC Manager could be installed. You need to restart the installer to try again." But when I restart, it simply tells me the same thing. What should I do?
...
#### *Question -*  When I try to install openPDC, I recieve an error "Cannot delete the existing openPDC."
...
#### *Question -*  I just installed the openPDCManager. Why isn't it working properly?
...
#### *Question -*  When I go to the webpage, I just get a white screen with an &quot;error on page&quot; icon on the statusbar.</h4>
...
#### *Question -*  The openPDCManager doesn't appear to be connecting to my database.
...
#### *Question -*  Why am I receiving a CrossDomainError?
...
#### *Question -*  What is the purpose of the local checkbox on the Manage Historians? Are there cases where they would not on the local network? What does this change when active?
...
#### *Question -*  The Specified service does not exist as an installed service
...
#### *Question -*  I receive network permissions or HTTP related errors
...
#### *Question -*  I noticed that my history file seems to stay the same size all of the time. Does it only store a certain amount of history?
...
#### *Question -*  How does the openPDC handle situations where a historian is not responding?
...
#### *Question -*  How does the openPDC handle the locally cached measurements?
...
#### *Question -*  How does Hadoop relate to the openPDC?
...
#### *Question -*  How are openPDC and Hadoop used in production?
